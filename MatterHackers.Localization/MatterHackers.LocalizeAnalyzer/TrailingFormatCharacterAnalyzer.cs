...
namespace MatterHackers.LocalizeAnalyzer
{
	...
	public class SpellingAnalyzer : DiagnosticAnalyzer
	{
		public const string DiagnosticId = "Spelling2";
		public const string ComplicatedUseID = "ComplicatedUseID2";

		private static readonly string Title = "Extract trailing formatter2";
		private static readonly string MessageFormat = "Trailing format characters2";
		private static readonly string Description = "Extract and concatenate trailing format characters2";
		private const string Category = "Localization";

		private static readonly Regex splitOnWhitespace = new Regex("\\s+", RegexOptions.Compiled);

		//private static Hunspell hunspell2 = null; // ("en_us.aff", "en_us.dic"));
		...
		//		if (hunspell2 == null)
		//		{
		//			hunspell2 = new NHunspell.Hunspell(); // "en_us.aff", "en_us.dic");
		//		}
			...
			if (identifier.Value.ValueText == "Localize")
			{
					...
					&& invocation.Expression is MemberAccessExpressionSyntax memberAccess)
				{
					if (memberAccess.Expression is LiteralExpressionSyntax literalExpression
						&& literalExpression.Token.ValueText?.Trim() is string line)
					{
						// Make sure every word is spelled correctly
						if (!splitOnWhitespace.Split(line).All(word => false)) // this.Speller.Spell(word)))
						{
							//var textInvokedOn = literalExpression.Token.ValueText;
							analysisContext.ReportDiagnostic(
								Diagnostic.Create(MoveColonRule, invocation.GetLocation()));
